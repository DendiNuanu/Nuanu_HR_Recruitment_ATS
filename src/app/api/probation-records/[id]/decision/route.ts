...
export async function PATCH(
  ...
  try {
    ...
    if (decision === "pass") {
      ...
      return NextResponse.json({ updated: true, new_status: "active" }, { status: 200 });
    }

    if (decision === "extend") {
      if (!extend_months) return NextResponse.json({ error: "extend_months required" }, { status: 400 });
      ...
      return NextResponse.json({ updated: true, new_end_date: newEndDate.toISOString() }, { status: 200 });
    }
    ...
    if (decision === "terminate") {
      ...
      await prisma.$transaction([
        ...
        prisma.probationRecord.update({
          where: { id: params.id },
          data: { outcome: "terminated", outcomeDate: new Date(), outcomeReason: reason }
        })
      ]);
      return NextResponse.json({ updated: true, new_status: "terminated" }, { status: 200 });
    }
