...
export async function PATCH(
  ...
  try {
    ...
    if (assigned_date !== undefined)
      updateData.assignedDate = assigned_date ? new Date(assigned_date) : null;
    if (notes !== undefined) updateData.notes = notes;
    if (status !== undefined) updateData.status = status;
    if (received_date !== undefined)
      updateData.receivedDate = received_date ? new Date(received_date) : null;
    if (returned_date !== undefined)
      updateData.returnedDate = returned_date ? new Date(returned_date) : null;
