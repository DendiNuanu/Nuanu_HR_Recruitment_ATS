...
export default function InterviewsClient({
  ...
  const [activeInterview, setActiveInterview] = useState<InterviewData | null>(
    null,
  );
  ...
  const [feedbackMode, setFeedbackMode] = useState<"form" | "result">("form");
  ...
  const handleFeedbackSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await submitInterviewFeedback({
        ...
        leadershipScore: feedbackData.leadershipScore,
      ...
      if (res.success) {
        ...
        setSubmittedFeedback({
          ...feedbackData,
          interviewerName: activeInterview.interviewerName,
  ...
  const openFeedbackForm = (interview: InterviewData) => {
  ...
  const openViewResults = async (interview: InterviewData) => {
    setActiveInterview(interview);
    setIsLoadingResult(true);
    setFeedbackMode("result");
    setSubmittedFeedback(null);
    setIsFeedbackOpen(true);
    const data = await getInterviewFeedback(interview.id);
    if (data) {
      const notesText = data.notes ?? "";
      const recruiterMatch = notesText.match(
        /\[RECRUITER NOTES\]\n([\s\S]*?)(?=\n\n\[|$)/,
      );
      const hrMatch = notesText.match(
        /\[HR NOTES - PRIVATE\]\n([\s\S]*?)(?=$)/,
      );
      setSubmittedFeedback({
        ...
        recruiterNotes: recruiterMatch ? recruiterMatch[1].trim() : "",
        hrNotes: hrMatch ? hrMatch[1].trim() : "",
            ...
            >
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex flex-1 items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-nuanu-gray-500">
                      {interview.position} • {interview.stage.replace("_", " ")}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-2 min-w-[200px]">
                  ...
                  <div className="flex items-center gap-2 text-sm text-nuanu-gray-600">
                    {interview.type === "video" ? (
                      <Video className="w-4 h-4 text-nuanu-gray-400" />
                    ) : (
                      <MapPin className="w-4 h-4 text-nuanu-gray-400" />
                    )}
                    {interview.location}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                  <div className="flex flex-col items-end gap-2">
                    ...
                    <span className="text-xs text-nuanu-gray-500">
              ...
              <div className="mt-4 pt-4 border-t border-nuanu-gray-100 flex gap-3">
                {interview.status === "completed" ? (
                  <button
                    onClick={() => openViewResults(interview)}
                    className="btn-primary text-sm py-1.5 px-4 bg-indigo-600 hover:bg-indigo-700 border-0"
                  >
                ...
                {interview.calendarSynced && interview.meetingLink && (
                  <a
                    ...
                    className="btn-primary text-sm py-1.5 px-4 ml-auto bg-blue-600 hover:bg-blue-700"
                  >
          ...
          {filteredInterviews.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-nuanu-gray-500 mt-1">
                Try adjusting your filters or schedule a new one
              </p>
            </div>
          )}
        </div>
            ...
            >
              <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-gray-50">
                <h2 className="text-lg font-bold text-nuanu-navy flex items-center gap-2">
                  {feedbackMode === "result" ? (
                    <>
                      <Star className="w-5 h-5 text-amber-500" /> Interview
                      Results: {activeInterview?.candidateName}
                    </>
                  ) : (
                    <>
                      <CheckCircle2 className="w-5 h-5 text-nuanu-emerald" />{" "}
                      Interview Feedback: {activeInterview?.candidateName}
                    </>
                  )}
                </h2>
                <button
              ...
              {feedbackMode === "form" ? (
                ...
                >
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <span className="text-lg font-bold text-nuanu-navy min-w-[2ch]">
                        ...
                        <div key={field.id}>
                          <div className="flex justify-between items-center mb-1">
                            ...
                            <span className="text-xs font-bold text-nuanu-navy">
                              {(feedbackData as any)[field.id]}/10
                            </span>
                          </div>
                          <input
                            type="range"
                  ...
                  <div>
                    ...
                    <textarea
                      value={feedbackData.recruiterNotes}
                      onChange={(e) =>
                        setFeedbackData({
                          ...feedbackData,
                          recruiterNotes: e.target.value,
                        })
                      }
                      className="input-field py-2 text-sm"
                      rows={2}
                      placeholder="Notes visible to recruiter team..."
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="input-field py-2 text-sm"
                      rows={2}
                      placeholder="Confidential HR internal notes..."
                    />
                  </div>
                ...
                <div className="flex items-center justify-center py-20">
                  <Loader2 className="w-8 h-8 text-nuanu-emerald animate-spin" />
                ...
                <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-16 h-16 rounded-2xl flex items-center justify-center text-white text-2xl font-black shadow-lg ${
                          (submittedFeedback?.overallRating ?? 0) >= 8
                            ? "bg-emerald-500"
                            : (submittedFeedback?.overallRating ?? 0) >= 5
                              ? "bg-amber-500"
                              : "bg-red-500"
                        }`}
                      >
                      ...
                      <div>
                        ...
                        <p className="text-lg font-black text-nuanu-navy">
                          out of 10
                        </p>
                      </div>
                    </div>
                    ...
                    <span
                      className={`px-6 py-3 rounded-full font-black uppercase text-sm tracking-wider ${
                        submittedFeedback?.recommendation === "hire"
                          ? "bg-emerald-100 text-emerald-700"
                          : submittedFeedback?.recommendation === "reject"
                    ...
                    {[
                      ...
                      <div key={key}>
                        <div className="flex justify-between mb-1">
                          ...
                          <span className="text-xs font-black text-nuanu-navy">
                            {(submittedFeedback as any)?.[key]}/10
                          </span>
                        </div>
                        <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${((submittedFeedback as any)?.[key] ?? 0) * 10}%`,
                            }}
                          />
                        </div>
                      </div>
                  ...
                  {/* Recruiter Notes */}
                  {submittedFeedback?.recruiterNotes && (
                  ...
                  {/* Submitted by / time */}
                  {submittedFeedback?.submittedAt && (
                    <p className="text-xs text-gray-400 text-center">
                      Submitted by{" "}
                      {submittedFeedback.interviewerName ?? "Interviewer"} on{" "}
                      {new Date(submittedFeedback.submittedAt).toLocaleString()}
                    </p>
                  )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-red-50/50">
                <h2 className="text-lg font-bold text-nuanu-navy flex items-center gap-2">
                  <Clock className="w-5 h-5 text-red-500" /> Reschedule
                  Interview
                </h2>
                <button
                  onClick={() => !isSubmitting && setIsRescheduleOpen(false)}
              ...
              <form onSubmit={handleRescheduleSubmit} className="p-6 space-y-4">
                <div className="p-4 bg-nuanu-gray-50 rounded-xl border border-nuanu-gray-100">
                  <p className="text-xs text-nuanu-gray-500 mb-1 uppercase font-bold tracking-wider">
                    Current Schedule
                  </p>
                  <p className="text-sm font-bold text-nuanu-navy">
                    {activeInterview
                      ? new Date(activeInterview.scheduledAt).toLocaleString(
                          [],
                ...
                <div>
                  <DateTimePicker
                    ...
                    onChange={(val) =>
                      setRescheduleData({ ...rescheduleData, scheduledAt: val })
                    }
                  />
                </div>
