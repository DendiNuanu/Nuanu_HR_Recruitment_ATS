...
export default function EmployeeDetailModal({
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [empRes, contractRes, docsRes, assetsRes, historyRes, probRes] =
        await Promise.all([
          ...
          fetch(`/api/probation-records/employee/${employeeId}`),
        ]);
  ...
  const handleQuickAssetStatus = async (
    assetId: string,
    newStatus: "received" | "returned",
  ) => {
    try {
      ...
      if (newStatus === "returned")
        body.returned_date = new Date().toISOString();
  ...
  const handleDecision = async (decision: string) => {
    ...
    if (decision === "extend") {
      ...
      if (
        !window.confirm(
          `Are you sure? This will mark ${employee?.user?.name} as a permanent employee.`,
        )
      )
        return;
    }
    ...
    try {
      const res = await fetch(
        `/api/probation-records/${probationRecord?.id}/decision`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
            ...
            <div className="p-6">
                ...
                <div className="space-y-6">
                  <div className="card flex items-start justify-between">
                    <div className="flex items-center gap-6">
                      ...
                      <div>
                        <h3 className="text-2xl font-bold text-nuanu-navy">
                          {employee.user.name}
                        </h3>
                  ...
                  {isEditingProfile && (
                    <div className="card">
                      <h4 className="font-bold text-nuanu-navy mb-4">
                        Edit Profile Info
                      </h4>
              ...
              {activeTab === "probation" && probationRecord && (
                <div className="space-y-6">
                  {/* Summary Card */}
                  <div className="card">
                    <h3 className="font-bold text-lg text-nuanu-navy mb-4">
                      Probation Overview
                    </h3>
                    <div className="grid grid-cols-3 gap-4 mb-4 text-sm">
                      <div>
                        ...
                        <span className="font-medium">
                          {formatDate(probationRecord.probationStart)}
                        </span>
                      </div>
                      <div>
                        <span className="text-gray-500 block text-xs">
                        ...
                        <span className="font-medium">
                          {formatDate(probationRecord.probationEndDate)}
                        </span>
                      </div>
                      <div>
                        <span className="text-gray-500 block text-xs">
                        ...
                        <span
                          className={`font-bold ${
                            Math.ceil(
                              (new Date(
                                probationRecord.probationEndDate,
                              ).getTime() -
                                new Date().getTime()) /
                                86400000,
                            ) <= 14
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        ...
                        style={{
                          width: `${Math.min(100, Math.max(0, ((new Date().getTime() - new Date(probationRecord.probationStart).getTime()) / (new Date(probationRecord.probationEndDate).getTime() - new Date(probationRecord.probationStart).getTime())) * 100))}%`,
                        }}
                      ></div>
                    </div>
                  </div>
                  ...
                  <div className="card">
                    ...
                    {showAddEval && (
                      <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 mb-6">
                        <h4 className="font-bold text-sm text-nuanu-navy mb-4">
                          New Evaluation
                        </h4>
                        <div className="grid grid-cols-2 gap-4 mb-4">
                          <div>
                            <label className="block text-xs font-semibold text-gray-600 mb-1">
                              Evaluated By (Manager/HR)
                            </label>
                            <input
                              type="text"
                              value={evalForm.evaluated_by}
                              onChange={(e) =>
                                setEvalForm({
                                  ...evalForm,
                                  evaluated_by: e.target.value,
                                })
                              }
                              className="input-field"
                        ...
                        <div className="flex justify-end gap-2 pt-4 border-t border-gray-200">
                          ...
                          <button
                            ...
                            disabled={!evalForm.evaluated_by}
                            className="btn-primary text-sm"
                          >
                            Save Evaluation
                          </button>
                    ...
                    probationRecord.evaluations.length > 0 ? (
                        ...
                        {probationRecord.evaluations.map((ev: any) => (
                          ...
                          >
                            <div className="flex justify-between items-start mb-2">
                              <div>
                                <p className="font-semibold text-sm text-nuanu-navy">
                  ...
                  {probationRecord.outcome === null &&
                    probationRecord.evaluations?.length > 0 && (
                      <div className="card bg-gray-50 border border-gray-200 shadow-inner">
                        <h3 className="font-bold text-nuanu-navy mb-2">
                          Final Probation Decision
                        </h3>
                        <p className="text-sm text-gray-500 mb-6">
                          Make a final decision to conclude this probation
                          period. This action is permanent.
                        </p>
                    ...
                    <div className="card bg-blue-50 border-blue-100">
                      ...
                      <p className="text-sm text-blue-700">
                        ...
                        <strong className="uppercase">
                          {probationRecord.outcome}
                        </strong>
                      </p>
                      {probationRecord.outcomeReason && (
                        <p className="text-sm text-blue-700 mt-2">
                          Reason: {probationRecord.outcomeReason}
                        </p>
                      )}
                    </div>
                  )}
                  ...
                  {contract ? (
                    <div className="card">
                      <div className="flex items-center justify-between mb-6">
                        <h3 className="font-bold text-lg text-nuanu-navy">
                          Contract Details
                        </h3>
                    ...
                    <div className="text-center py-16 card">
                      ...
                      <Link
                        href="/dashboard/onboarding"
                        className="text-blue-500 hover:underline text-sm font-medium mt-2 block"
                      >
                        Go to Onboarding →
                      </Link>
                    </div>
                  ...
                  {contract ? (
                    <div className="card max-w-lg mx-auto">
                      <div className="flex items-center justify-between mb-6">
                        ...
                        <span className="badge bg-blue-100 text-blue-700 uppercase font-bold text-xs">
                          {contract.salaryType}
                        </span>
                      </div>
                      <div className="space-y-3 font-mono text-sm">
                        <div className="flex justify-between items-center">
                          ...
                          <span>
                            Rp {Number(contract.basicSalary).toLocaleString()}
                          </span>
                        </div>
                        {Number(contract.mealAllowance) > 0 && (
                          <div className="flex justify-between items-center">
                            ...
                            <span>
                              Rp{" "}
                              {Number(contract.mealAllowance).toLocaleString()}
                            </span>
                          </div>
                        )}
                        ...
                        {Number(contract.healthAllowance) > 0 && (
                          <div className="flex justify-between items-center">
                            <span className="text-gray-600 font-sans">
                              + Health Allowance
                            </span>
                            <span>
                              Rp{" "}
                              {Number(
                                contract.healthAllowance,
                              ).toLocaleString()}
                            </span>
                          </div>
                        )}
                        {Number(contract.otherAllowanceAmount) > 0 && (
                          <div className="flex justify-between items-center">
                            <span className="text-gray-600 font-sans">
                              +{" "}
                              {contract.otherAllowanceLabel ||
                                "Other Allowance"}
                            </span>
                            <span>
                              Rp{" "}
                              {Number(
                                contract.otherAllowanceAmount,
                              ).toLocaleString()}
                            </span>
                          </div>
                        )}
                        ...
                        <div className="flex justify-between items-center text-base font-bold text-nuanu-navy">
                          ...
                          <span>
                            Rp{" "}
                            {(
                              Number(contract.basicSalary) +
                              Number(contract.mealAllowance) +
                              Number(contract.transportAllowance) +
                              Number(contract.healthAllowance) +
                              Number(contract.otherAllowanceAmount)
                            ).toLocaleString()}
                          </span>
                        </div>
                      </div>
                    ...
                    <div className="text-center py-16 card">
                      ...
                      <p className="text-sm text-gray-400 mt-1">
                        Please complete the New Hire Confirmation form.
                      </p>
                    </div>
                  )}
                </div>
                              ...
                              return (
                                <tr key={doc.id}>
                                  ...
                                  <td className="px-4 py-3">
                                    <span
                                      className={`px-2 py-0.5 rounded-md text-[10px] font-bold border uppercase tracking-wider ${sc.cls}`}
                                    >
                                      {sc.label}
                                    </span>
                                    {doc.rejectionReason && (
                                  ...
                                  <td className="px-4 py-3 text-right">
                                    {(doc.verificationStatus === "uploaded" ||
                                      ...
                                      <div className="flex items-center justify-end gap-2">
                                        <button
                                          onClick={() =>
                                            handleVerifyDoc(doc.id, "verified")
                                          }
                                          className="btn-primary py-1 px-2 text-[10px] bg-emerald-600 border-emerald-600"
                                        >
                                          Approve
                                        </button>
                                        <button
                                          ...
                                          className="btn-secondary py-1 px-2 text-[10px] text-red-600 border-red-200 hover:bg-red-50"
                                        >
                                          Reject
                                        </button>
                                      </div>
              ...
              {activeTab === "history" && (
                <div className="space-y-6">
                  {history.length > 0 ? (
                    <div className="card">
                      <h3 className="font-bold text-nuanu-navy mb-6">
                        Activity Timeline
                      </h3>
                      <div className="relative border-l-2 border-gray-200 ml-4 space-y-8 pb-4">
                        {history.map((event, i) => {
                          ...
                          if (event.icon === "file_signature")
                            EventIcon = FileText;
                          if (event.icon === "file_text") EventIcon = FileText;
                          if (event.icon === "check_circle")
                          ...
                          return (
                            <div key={i} className="relative pl-6">
                              ...
                              <div className="text-sm">
                                <p className="font-medium text-nuanu-navy">
                                  {event.description}
                                </p>
