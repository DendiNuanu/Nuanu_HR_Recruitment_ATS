...
export default async function AIScoringPage() {
  ...
  applications.forEach((app) => {
    if (
      app.candidateScore?.missingKeywords &&
      app.candidateScore.missingKeywords.length > 0
    ) {
  ...
  const missingKeywordAlerts: MissingKeywordAlert[] = Array.from(
    vacancyAlertsMap.entries(),
  ).map(([vacancyTitle, keywordSet]) => ({
    vacancyTitle,
  ...
  const topMatches: TopMatch[] = applications.map((app) => {
    ...
    return {
      ...
      skills:
        ...
        app.candidateScore.matchedKeywords.length > 0
          ? app.candidateScore.matchedKeywords
  ...
  const vacancies = Array.from(
    new Set(applications.map((app) => app.vacancy.title)),
  ).sort();
