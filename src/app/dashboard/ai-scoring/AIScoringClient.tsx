      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-8 bg-white p-10 rounded-[2.5rem] border border-nuanu-gray-100 shadow-xl shadow-nuanu-gray-200/20 mb-4">
        <div className="flex-1 space-y-4">
          ...
          <p className="text-sm text-nuanu-gray-500 font-bold max-w-2xl">
            Automatically evaluating candidate resumes against job requirements
            using advanced neural scoring models.
          </p>
        </div>
              ...
              <div className="flex items-center justify-between mb-8">
                <div>
                  ...
                  <p className="text-indigo-200 text-sm font-medium">
                    Processing large-scale candidate datasets...
                  </p>
                </div>
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6 bg-white p-8 rounded-[2rem] border border-nuanu-gray-100 shadow-xl shadow-nuanu-gray-200/20">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h2 className="text-2xl font-black text-nuanu-navy tracking-tight">
                Recommendations
              </h2>
              <p className="text-[10px] font-black text-nuanu-gray-400 uppercase tracking-[0.2em] mt-1">
                Top Matched Talent
              </p>
            </div>
          </div>
        ...
        <div className="space-y-6">
                ...
                <div className="p-8 flex flex-col lg:flex-row gap-10 items-center">
                  ...
                  <div className="flex-shrink-0">
                    <div className="relative w-36 h-36 group-hover:scale-105 transition-transform duration-500">
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <span className="text-3xl font-black text-nuanu-navy leading-none tracking-tighter">
                          {candidate.score}%
                        </span>
                        <span className="text-[10px] text-nuanu-gray-400 font-black uppercase tracking-widest mt-1.5">
                          Match
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="flex-1 text-center lg:text-left space-y-6 w-full">
                    <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
                      <div>
                        ...
                        <p className="text-base font-bold text-nuanu-gray-500 mt-2">
                          ...
                          <span className="text-nuanu-gray-200 mx-1">at</span>{" "}
                          {candidate.currentCompany}
                        </p>
                      </div>
                      <div className="flex justify-center xl:justify-end">
                        <span
                          className={`px-6 py-2 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-lg ${
                            candidate.score >= 85
                    ...
                    <div className="space-y-4">
                      <p className="text-[9px] font-black text-nuanu-gray-300 uppercase tracking-[0.3em]">
                        Neural Skillset Analysis
                      </p>
                      <div className="flex flex-wrap justify-center lg:justify-start gap-3">
                        {candidate.skills.map((skill) => (
                          <span
                            key={skill}
                            className="px-4 py-2 bg-nuanu-gray-50 text-nuanu-navy text-xs font-black rounded-xl border border-nuanu-gray-100 flex items-center gap-2.5 transition-all hover:bg-white hover:shadow-md"
                          >
          ...
          {filteredMatches.length === 0 && (
            <div className="w-full card p-24 flex flex-col items-center justify-center text-center bg-nuanu-gray-50/20 border-dashed border-2 border-nuanu-gray-200 rounded-[3rem] shadow-inner">
              ...
              <h3 className="text-3xl font-black text-nuanu-navy mb-4 tracking-tight">
                No Matches Found
              </h3>
              <p className="text-base text-nuanu-gray-500 max-w-md font-medium leading-relaxed">
                The intelligence engine hasn't mapped any candidates to this
                role yet. Deploy the neural scanner to begin.
              </p>
              <button
                onClick={handleScan}
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            >
              <div className="p-10 border-b border-nuanu-gray-100 flex items-center justify-between bg-white">
                <div className="flex items-center gap-6">
                  <div className="w-16 h-16 rounded-2xl bg-indigo-600 flex items-center justify-center text-white font-black text-3xl shadow-xl">
                    {selectedAnalysis.application.candidate.name
                      .substring(0, 2)
                      .toUpperCase()}
                  </div>
                  <div>
                    <h3 className="text-3xl font-black text-nuanu-navy tracking-tight">
                      {selectedAnalysis.application.candidate.name}
                    </h3>
                    <p className="text-sm font-bold text-nuanu-gray-400 uppercase tracking-widest mt-1">
                      {selectedAnalysis.application.vacancy.title}{" "}
                      <span className="mx-3 text-nuanu-gray-200">|</span> AI
                      Neural Score
                    </p>
                  </div>
                </div>
                <button
                  onClick={() => setSelectedAnalysis(null)}
                  className="p-4 rounded-2xl hover:bg-nuanu-gray-100 transition-all"
                >
              ...
              <div className="flex-1 overflow-y-auto p-12 space-y-16">
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
                  {[
                    ...
                    {
                      label: "Hard Skills",
                      score: selectedAnalysis.hardSkillsScore,
                      color: "text-emerald-600",
                      bg: "bg-emerald-50/50",
                    },
                    {
                      label: "Soft Skills",
                      score: selectedAnalysis.softSkillsScore,
                    ...
                    },
                    {
                      ...
                      score: selectedAnalysis.formatScore,
                      color: "text-teal-600",
                      bg: "bg-teal-50/50",
                    },
                  ].map((stat) => (
                    <div
                      key={stat.label}
                      className={`text-center p-6 rounded-[2.5rem] ${stat.bg} border border-white shadow-xl transition-all hover:scale-105`}
                    >
                      <div className={`text-4xl font-black ${stat.color}`}>
                        {stat.score}%
                      </div>
                      <div className="text-[10px] text-nuanu-gray-400 font-black uppercase tracking-widest mt-3 leading-tight">
                        {stat.label}
                      </div>
                    </div>
                  ))}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                  <div className="space-y-8">
                    <h4 className="text-2xl font-black text-nuanu-navy flex items-center gap-4">
                      <Sparkles className="w-6 h-6 text-emerald-500" /> Neural
                      Strengths
                    </h4>
                    <div className="space-y-4">
                      {selectedAnalysis.strengths.map(
                        (s: string, i: number) => (
                          <div
                            key={i}
                            className="flex items-start gap-5 p-6 rounded-3xl bg-emerald-50/30 border border-emerald-100/50 text-base font-bold text-emerald-900 shadow-sm transition-all hover:bg-white hover:shadow-xl"
                          >
                  ...
                  <div className="space-y-8">
                    <h4 className="text-2xl font-black text-nuanu-navy flex items-center gap-4">
                      <AlertCircle className="w-6 h-6 text-amber-500" /> Neural
                      Gaps
                    </h4>
                    <div className="flex flex-wrap gap-4">
                      {selectedAnalysis.missingKeywords.map((kw: string) => (
                        <span
                          key={kw}
                          className="px-6 py-3 bg-amber-50 text-amber-800 border border-amber-100 text-xs rounded-2xl font-black uppercase tracking-tight shadow-md hover:scale-105 transition-transform"
                        >
                          {kw}
                        </span>
                      ))}
                ...
                <div className="p-10 rounded-[3rem] bg-nuanu-navy text-white space-y-8 shadow-3xl relative overflow-hidden">
                  ...
                  <h4 className="text-3xl font-black flex items-center gap-6 tracking-tight relative z-10">
                    <Brain className="w-8 h-8 text-emerald-400" /> Neural
                    Recommendation
                  </h4>
                  <ul className="space-y-5 relative z-10">
                    {selectedAnalysis.recommendations.map(
                      (r: string, i: number) => (
                        <li
                          key={i}
                          className="flex items-start gap-6 text-lg font-medium text-indigo-100 leading-relaxed"
                        >
              ...
              <div className="p-10 border-t border-nuanu-gray-100 bg-white/80 backdrop-blur-md flex justify-end gap-6">
                <button
                  onClick={() => setSelectedAnalysis(null)}
                  className="px-10 py-5 text-sm font-black uppercase tracking-[0.3em] text-nuanu-gray-400 hover:text-nuanu-navy transition-all"
                >
                  Dismiss
                </button>
                <button
