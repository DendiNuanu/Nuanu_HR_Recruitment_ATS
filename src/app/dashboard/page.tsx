...
async function getDashboardMetrics(): Promise<DashboardMetrics> {
  ...
  const offerAcceptanceRate =
    offers.length > 0 ? Math.round((acceptedOffers / offers.length) * 100) : 0;
  ...
  const candidateSourceBreakdown = Array.from(mergedSources.entries())
    .map(([key, count]) => ({
      ...
      count,
      percentage: mergedTotal > 0 ? Math.round((count / mergedTotal) * 100) : 0,
    }))
  ...
  const linkedinRate =
    totalHiresCount > 0
      ...
      : 0;
  const seekRate =
    totalHiresCount > 0 ? Math.round((seekHires / totalHiresCount) * 100) : 0;
  ...
  for (const row of hiredBySource) {
    ...
    if (!key) continue;
    mergedHiredBySource.set(
      key,
      (mergedHiredBySource.get(key) ?? 0) + row._count,
    );
  }
  ...
  const ttfDays = hiredApplications
    ...
    .map((a) => {
      ...
      return Math.round(Math.abs(end.getTime() - start.getTime()) / 86_400_000);
    })
    .filter((d) => d > 0 && d < 500);
  const avgTimeToFill =
    ttfDays.length > 0
  ...
  const quarterlyRates = [1, 2, 3, 4].map((q) => {
    const qHires = hiredApplications.filter((a) => {
      const hiredAt = a.offer?.respondedAt ?? a.updatedAt;
      return hiredAt.getFullYear() === currentYear && quarterOf(hiredAt) === q;
    });
  ...
  const genderBreakdown = Array.from(genderMap.entries())
    ...
    .sort((a, b) => b.count - a.count);
  const totalAge = Array.from(ageMap.values()).reduce((a, b) => a + b, 0) || 1;
