...
export default function RequisitionsClient({
  ...
  const isSuperAdmin = initialUser.roles?.some(
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      ...
      case "CANCELLED":
      case "REJECTED":
        return "bg-red-500 text-white border-red-400 shadow-red-500/20";
      case "ON_HOLD":
        return "bg-blue-500 text-white border-blue-400 shadow-blue-500/20";
      case "FULFILLED":
        return "bg-teal-500 text-white border-teal-400 shadow-teal-500/20";
      default: // DRAFT / PENDING
        return "bg-amber-500 text-white border-amber-400 shadow-amber-500/20";
    }
  };

  const getConcludedStyle = (status: string) => {
    switch (status) {
      case "PUBLISHED":
      case "APPROVED":
        return {
          wrap: "bg-emerald-50/50 border-emerald-100",
          text: "text-emerald-700",
        };
      case "CANCELLED":
      case "REJECTED":
        return { wrap: "bg-red-50/50 border-red-100", text: "text-red-700" };
      case "ON_HOLD":
        return { wrap: "bg-blue-50/50 border-blue-100", text: "text-blue-700" };
      case "FULFILLED":
        return { wrap: "bg-teal-50/50 border-teal-100", text: "text-teal-700" };
      default:
        return {
  ...
  const filtered = Array.isArray(requisitions)
    ? requisitions.filter((r) => {
        const matchesSearch =
          r.vacancy.title.toLowerCase().includes(search.toLowerCase()) ||
          r.vacancy.code.toLowerCase().includes(search.toLowerCase());
        const normalised = r.status.toLowerCase().replace("_", "");
        const matchesFilter =
          filter === "all" ||
          r.status.toLowerCase() === filter ||
          normalised === filter.replace("_", "");
    ...
    <div className="pb-20 relative">
      <div className="space-y-8 animate-fade-in">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <h1 className="text-4xl font-black text-nuanu-navy tracking-tight uppercase">
              Job Requisitions
            </h1>
            <p className="text-nuanu-gray-500 mt-1 font-semibold uppercase tracking-wider text-xs">
              Internal Position Creation & Approval Pipeline
            </p>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex items-center gap-1.5 bg-white p-1.5 border border-nuanu-gray-200 rounded-2xl shadow-sm flex-wrap">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-40 space-y-6">
            ...
            <p className="text-nuanu-gray-400 font-black animate-pulse uppercase tracking-[0.3em] text-[10px]">
          ...
          <div className="bg-white rounded-[3rem] border border-nuanu-gray-200 p-32 text-center shadow-sm">
            ...
            <h3 className="text-3xl font-black text-nuanu-navy uppercase tracking-tight">
              No Active Requisitions
            </h3>
            <p className="text-nuanu-gray-400 mt-4 max-w-sm mx-auto font-bold uppercase tracking-widest text-xs">
              Initiate a requisition to begin the recruitment process.
            </p>
            <button
              onClick={() => setIsModalOpen(true)}
              className="btn-secondary mt-12 font-black border-2 px-12 py-5 rounded-2xl uppercase tracking-widest text-xs"
            >
            ...
            {filtered.map((req) => {
              ...
              return (
                <motion.div
                  layout
                  key={req.id}
                  className="group bg-white rounded-3xl border border-nuanu-gray-200 overflow-hidden transition-all shadow-sm hover:shadow-[0_40px_80px_rgba(10,22,40,0.08)] hover:border-nuanu-emerald/40"
                >
                  <div className="p-6 md:p-8">
                    <div className="flex flex-col lg:flex-row justify-between gap-6">
                      <div className="flex gap-6 items-start">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-4 mb-2 flex-wrap">
                            <h3 className="text-xl font-black text-nuanu-navy tracking-tight truncate">
                              {req.vacancy.title}
                            </h3>
                            <span className="text-[10px] font-black bg-nuanu-navy text-white px-3 py-1 rounded-full uppercase tracking-[0.2em] shadow-lg shadow-nuanu-navy/20">
                              #{req.vacancy.code}
                            </span>
                          </div>
                      ...
                      <div className="flex flex-col items-end gap-5 flex-shrink-0">
                        <span
                          className={`px-8 py-3 rounded-full text-[11px] font-black uppercase tracking-[0.3em] shadow-xl border-2 transition-all ${getStatusStyle(req.status)}`}
                        >
                        ...
                        <button
                          ...
                          className="flex items-center gap-3 text-xs font-black text-nuanu-emerald hover:text-nuanu-navy transition-all group/btn uppercase tracking-[0.2em] bg-nuanu-gray-50/50 px-6 py-3 rounded-xl border border-transparent hover:border-nuanu-emerald/20"
                        >
                          {selectedId === req.id
                            ? "HIDE DETAILS"
                            : "VIEW PIPELINE"}
                          <ChevronRight
                            className={`w-5 h-5 transition-transform duration-500 ${selectedId === req.id ? "rotate-90" : "group-hover/btn:translate-x-1"}`}
                          />
                        </button>
                      </div>
                    </div>
                  ...
                  <AnimatePresence>
                    {selectedId === req.id && (
                      <motion.div
                        ...
                        className="overflow-hidden bg-nuanu-gray-50/40 border-t border-nuanu-gray-100"
                      >
                        <div className="p-8 md:p-10">
                          <div className="mb-10">
                            <h4 className="text-[10px] font-black text-nuanu-gray-400 uppercase tracking-[0.4em] mb-8 text-center">
                          ...
                          <div className="mt-10 w-full px-4">
                              ...
                              <div className="bg-white p-8 rounded-[2.5rem] border border-nuanu-gray-200 shadow-[0_50px_100px_rgba(10,22,40,0.1)] ring-1 ring-black/5 w-full">
                                {!isMyTurn ? (
                                  <div className="flex flex-col items-center justify-center py-16 text-center space-y-6">
                                    ...
                                    <div>
                                      <p className="text-lg font-black text-nuanu-navy uppercase tracking-[0.3em]">
                                        Phase Restricted
                                      </p>
                                      <p className="text-sm text-nuanu-gray-400 mt-3 max-w-xs mx-auto font-bold uppercase tracking-widest leading-loose">
                                        Waiting for{" "}
                                        <span className="text-nuanu-emerald">
                                          {currentRole}
                                        </span>{" "}
                                  ...
                                  <div className="space-y-10">
                                    <div className="flex items-center gap-4">
                                      ...
                                      <div>
                                        <h4 className="text-base font-black text-nuanu-navy uppercase tracking-tight">
                                          Your Decision Required
                                        </h4>
                                        <p className="text-[9px] font-bold text-nuanu-gray-400 uppercase tracking-widest mt-1">
                                          Review the requisition and provide
                                          final feedback
                                        </p>
                                      </div>
                                    </div>
                                    ...
                                    <div className="space-y-3">
                                      <label className="text-[9px] font-black text-nuanu-gray-400 uppercase tracking-[0.2em] block ml-1">
                                        Decision Rationale
                                      </label>
                                      <textarea
                                        value={comment}
                            ...
                            {!isDraft(req.status) &&
                              (() => {
                                const s = getConcludedStyle(req.status);
                                return (
                                  <div
                                    className={`p-12 rounded-[3rem] border-2 text-center shadow-inner ${s.wrap}`}
                                  >
                                    <p
                                      className={`text-sm font-black uppercase tracking-[0.4em] ${s.text}`}
                                    >
                                      STATUS: {getStatusDisplay(req.status)}
                                    </p>
                                    <p className="text-nuanu-gray-500 mt-4 font-bold uppercase tracking-widest text-[10px]">
                                      This requisition cycle is now archived.
                                    </p>
                                  </div>
                                );
                              })()}
              ...
              >
                ...
                <div className="flex-1 overflow-y-auto p-12 md:p-16 space-y-16 custom-scrollbar bg-white w-full">
                  <form
                    ...
                    className="space-y-20 w-full max-w-[1200px] mx-auto"
                  >
                    {/* Section 1: Core Details */}
                    <div className="space-y-6">
                      <div className="flex items-center justify-between border-b-2 border-nuanu-gray-50 pb-4">
                        <div className="flex items-center gap-4">
                          <div className="w-8 h-8 rounded-lg bg-indigo-600 text-white flex items-center justify-center font-black text-sm shadow-[0_10px_25px_rgba(79,70,229,0.3)]">
                            01
                          </div>
                          <h3 className="text-lg font-black text-nuanu-navy tracking-tight uppercase">
                            Strategic Position Info
                          </h3>
                        </div>
                        <span className="text-[8px] font-black text-nuanu-gray-300 uppercase tracking-[0.5em]">
                          Phase One
                        </span>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                          <label className="text-[9px] font-black text-nuanu-gray-400 uppercase tracking-[0.25em] block ml-1">
                            Proposed Job Title{" "}
                            <span className="text-nuanu-emerald font-black">
                              *
                            </span>
                          </label>
                          <div className="relative group">
                            ...
                            <input
                              required
                              type="text"
                              placeholder="e.g. HEAD OF GROWTH DESIGN"
                              ...
                              <button
                                ...
                                className={`flex-1 rounded-xl text-[8px] font-black uppercase tracking-[0.1em] border-2 transition-all duration-300 ${
                                  formData.employmentType === type
                                    ? "bg-nuanu-navy text-white border-nuanu-navy shadow-lg scale-[1.02] z-10"
                                    : "bg-white text-nuanu-gray-400 border-nuanu-gray-100 hover:border-nuanu-navy/20"
                                }`}
                              >
                                {type}
                              </button>
                        ...
                        <div className="space-y-2 md:col-span-2">
                          <label className="text-[9px] font-black text-nuanu-gray-400 uppercase tracking-[0.25em] block ml-1">
                            Budget Allocation (IDR Monthly)
                          </label>
                          <div className="grid grid-cols-2 gap-4">
                            <div className="relative group">
                              <span className="absolute left-4 top-1/2 -translate-y-1/2 font-black text-nuanu-gray-300 text-base group-focus-within:text-nuanu-emerald transition-colors">
                                Rp
                              </span>
                              <input
                                type="number"
                    ...
                    <div className="space-y-6">
                      <div className="flex items-center justify-between border-b-2 border-nuanu-gray-50 pb-4">
                        <div className="flex items-center gap-4">
                          <div className="w-8 h-8 rounded-lg bg-amber-500 text-white flex items-center justify-center font-black text-sm shadow-[0_10px_25px_rgba(245,158,11,0.3)]">
                            02
                          </div>
                          <h3 className="text-lg font-black text-nuanu-navy tracking-tight uppercase">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                          ...
                          <div className="flex gap-2 h-12">
                            {["New Position", "Replacement", "Expansion"].map(
                              (type) => (
                                <button
                                  key={type}
                                  type="button"
                                  ...
                                  className={`flex-1 rounded-xl text-[8px] font-black uppercase tracking-[0.1em] border-2 transition-all duration-300 ${
                                    formData.justificationType === type
                                      ? "bg-nuanu-navy text-white border-nuanu-navy shadow-lg scale-[1.02] z-10"
                                      : "bg-white text-nuanu-gray-400 border-nuanu-gray-100 hover:border-nuanu-navy/20"
                                  }`}
                                >
                        ...
                        {formData.justificationType === "Replacement" && (
                          <div className="space-y-2 animate-in fade-in slide-in-from-top-4 duration-500">
                            <label className="text-[9px] font-black text-nuanu-gray-400 uppercase tracking-[0.25em] block ml-1">
                              Legacy Personnel Name
                            </label>
                            <input
                              type="text"
                              placeholder="WHO ARE WE REPLACING?"
                        ...
                        <div className="space-y-2 md:col-span-2">
                          ...
                          <textarea
                            required
                            placeholder="ELABORATE ON THE SPECIFIC BUSINESS NEED, PROJECT SCOPE, OR DEPARTMENTAL GAP THIS ROLE WILL ADDRESS..."
                    ...
                    <div className="space-y-6">
                      <div className="flex items-center justify-between border-b-2 border-nuanu-gray-50 pb-4">
                        <div className="flex items-center gap-4">
                          <div className="w-8 h-8 rounded-lg bg-nuanu-emerald text-white flex items-center justify-center font-black text-sm shadow-[0_10px_25px_rgba(16,185,129,0.3)]">
                            03
                          </div>
                          <h3 className="text-lg font-black text-nuanu-navy tracking-tight uppercase">
                            Operational Scope
                      ...
                      <div className="space-y-3">
                        {formData.responsibilities.map((resp, index) => (
                          <div
                            key={index}
                            className="flex gap-4 animate-in fade-in slide-in-from-left-6 duration-500"
                          ...
                          >
                            <div className="flex-1 relative group">
                              ...
                              <input
                                type="text"
                                placeholder={`KEY RESPONSIBILITY PILLAR ${index + 1}`}
                                value={resp}
                                onChange={(e) => {
                                  const newResp = [
                    ...
                    <div className="space-y-6">
                      <div className="flex items-center justify-between border-b-2 border-nuanu-gray-50 pb-4">
                        <div className="flex items-center gap-4">
                          <div className="w-8 h-8 rounded-lg bg-purple-600 text-white flex items-center justify-center font-black text-sm shadow-[0_10px_25px_rgba(147,51,234,0.3)]">
                            04
                          </div>
                          <h3 className="text-lg font-black text-nuanu-navy tracking-tight uppercase">
                            Talent Matrix
                        ...
                        <div className="space-y-4 md:col-span-2">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {formData.requiredSkills.map((skill, index) => (
                              <div
                                key={index}
                                className="flex gap-2 animate-in fade-in slide-in-from-right-6 duration-500"
                              >
                                <div className="flex-1 relative group">
                                  ...
                                  <input
                                    required
                                    type="text"
                                    placeholder={`CRITICAL COMPETENCY ${index + 1}`}
                                    value={skill}
                                    onChange={(e) => {
                                      const newSkills = [
                    ...
                    <div className="bg-nuanu-navy text-white p-6 rounded-3xl shadow-xl flex flex-col md:flex-row gap-6 items-center relative overflow-hidden group">
                      ...
                      <div className="flex-1 text-center md:text-left">
                        <p className="text-sm font-black uppercase tracking-[0.4em] mb-1">
                          Governance Protocol
                        </p>
                        <p className="text-[10px] text-nuanu-gray-300 leading-relaxed font-bold uppercase tracking-widest">
                          SUBMISSION INITIATES A FORMAL GOVERNANCE WORKFLOW
                          ACROSS <span className="text-white">MANAGEMENT</span>,{" "}
                          <span className="text-white">HRBP</span>, AND{" "}
                          <span className="text-white">FINANCE CONTROL</span>.
                        </p>
                      </div>
                    </div>
                  </form>
