            ...
            <AnimatePresence>
              {showUserMenu && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0, scale: 1 }}
                  exit={{ opacity: 0, y: 8, scale: 0.95 }}
                  className="absolute right-0 top-12 w-64 bg-white rounded-xl shadow-xl border border-nuanu-gray-200 overflow-hidden py-1"
                >
                  <div className="px-4 py-3 border-b border-nuanu-gray-100 bg-nuanu-gray-50/50">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-nuanu-emerald to-nuanu-teal flex items-center justify-center text-white text-sm font-bold shrink-0">
                        {currentUser?.name
                          ? getInitials(currentUser.name)
                          : "??"}
                      </div>
                      <div className="min-w-0">
                        <p className="text-sm font-bold text-nuanu-navy truncate">
                          {currentUser?.name || "Unknown User"}
                        </p>
                        <p className="text-[10px] text-nuanu-gray-400 font-medium uppercase tracking-wider truncate">
                          {currentUser?.roles?.[0]?.replace(/-/g, " ") ||
                            "User"}
                        </p>
                        <p className="text-[10px] text-nuanu-gray-400 truncate">
                          {currentUser?.email || ""}
                        </p>
                      </div>
                    </div>
